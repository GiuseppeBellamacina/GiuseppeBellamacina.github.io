<script lang="ts">
	...
	function createFirefly() {
		...
		// Durate casuali per ogni lucciola
		const lifeTime = Math.random() * 30000 + 15000; // 15-45 secondi di vita
	...
	function maintainFireflyPopulation() {
		...
		// Mantieni il numero tra 85% e 100% del massimo
		const minFireflies = Math.floor(maxFireflies * 0.85);

		if (current < minFireflies) {
			// Aggiungi qualche lucciola
			const toAdd = Math.min(5, maxFireflies - current); // Max 5 alla volta
			for (let i = 0; i < toAdd; i++) {
				setTimeout(() => createFirefly(), Math.random() * 1000);
			}
		}
	}
	...
	onMount(() => {
		...
		if (showFireflies) {
			...
			// Crea lucciole iniziali gradualmente
			const initialCount = Math.floor(maxFireflies * 0.9); // Parti con il 90%
			for (let i = 0; i < initialCount; i++) {
				setTimeout(() => createFirefly(), Math.random() * 3000); // Nei primi 3 secondi
			}

			// Controlla ogni 1.5 secondi se servono altre lucciole
			checkInterval = setInterval(maintainFireflyPopulation, 1500);
